...
2. Start the container using the pinned image tag:
   ...
   ```bash
   IMAGE_TAG=sha-82ceed5 docker compose up
   ```

   This:
   - Uses image `ruk2712/dsci-310-group-17:sha-82ceed5` (pinned to this project's latest release)
   ...
   > **Alternative tags:** All available image tags (including newer releases) are listed on [Docker Hub](https://hub.docker.com/r/ruk2712/dsci-310-group-17/tags). To use a different version, replace `sha-82ceed5` with any tag from that page, e.g.:
   > ```bash
